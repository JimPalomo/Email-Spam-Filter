...
void remDotCom(string domain, string& subDomain);
...
// Purpose: function that removes the ending of the domain (e.g. input: aol.com; output: aol) 
// Parameters: email (the email is being parsed), username (the username that is going to be updated), domain (the domain that is going to be updated)
// Return Value: the function returns the new domain (nicknamed subDomain) through pass by reference
void remDotCom(string domain, string& subDomain) {
    int cut = domain.find(".");
    subDomain = domain.substr(0,cut);   // get all before the . from .com
}

// Purpose: function that gets the current word's specific domain and username
// Parameters: curWord (input the current word being observed through the binary search), curWordUser (the username from the parse), curWordDom (the domain from the parse)
// Return Value: returns the current word's username and domain in binary search that is being compared through pass by reference
void getCurWordUserDom(string curWord, string& curWordUser, string& curWordDom) {
    int cut = curWord.find(":");
    curWordDom = curWord.substr(0, cut);    // get all before colon 
    curWordUser = curWord.substr(cut+1);    // get all after colon
...
// Return Value: returns the spam vector and uses pass by reference to limit the number of accesses
bool binarySearch(ourvector<string>& spamVect, string email) {
    ...
    string subDomain;                   // domain but with .com removed
    string curWord;                     // the current word used for comparison with the user's email inside the binary search
    string curWordUser;                 // the current word's username
    string curWordDom;                  // the current word's domain
    string domUser;                     // variable representing domain.com:username, used for binary search
    string checkStar;                   // variable representing domain.com:*, used for binary search
    ...
    remDotCom(domain, subDomain);                   // calls remove dot com function to remove the .com from the domain which is then used below
    ...
    checkStar = domain + ":" + "*";                 // concatenates the domain and a "*" and variable is used in the binary search for comparison


    if (username == subDomain) {                    // checks to see if the username is equal to the domain (no .com) and if they are then the email is not spam
        return 0;
    }
    ...
    while (l <= r) {                                // binary search starts here (uses a while loop and does not stop until l <= r condition is broken)
        ...
        if (checkStar == curWord) {                 // checks to see if the current word contains a star and if the midpoint also, returns spam
            return 1;
        }
        ...
        if (domUser > curWord) {                    // if the domain.com:username does not match the current's word domain.com:username then readjust & redo binary search until found
            ...
            return 1;                               // prevents infinite looping
        }
    }
...
// Purpose: function that returns the binary search's boolean value which is used to determine if an email is a spam or not
